...
void forward_move(int y_or_x, bool to_vertical) {
    ...
    for (int s = 0, e = 0; s < N - 1; s++) {
        e = s;
        while (++e < N && *tile(s) == 0) move(tile(s), tile(e));
        e = s + 1;
        while (++e < N && *tile(s + 1) == 0) move(tile(s + 1), tile(e));
        if (*tile(s) == *tile(s + 1)) {
            *tile(s) *= 2;
            *tile(s + 1) = 0;
        }
...
void backward_move(int y_or_x, bool to_vertical) {
    ...
    for (int s = N - 1, e = N - 1; s > 0; s--) {
        e = s;
        while (--e >= 0 && *tile(s) == 0) move(tile(s), tile(e));
        e = s - 1;
        while (--e >= 0 && *tile(s - 1) == 0) move(tile(s - 1), tile(e));
        if (*tile(s) == *tile(s - 1)) {
            *tile(s) *= 2;
            *tile(s - 1) = 0;
        }
