...
입력이 들어온 순서대로 숫자와 숫자가 들어온 순서(인덱스)를 저장해 둔다.

세그트리에 원하는 구간을 인덱스 정보로 탐색하여 구간 내에 몇 개의 수가 남아있는지를 센다.

이때, **트리의 리프노드는 1로 채워져 있고 인덱스에 해당하는 수를 제거했다면 0으로 업데이트**한다.

구간 합 세그트리를 이용하여 홀수 단계에서는 sum(0, index[n]), 짝수 단계에서는 sum(index[n], N-1) 의 구간 합을 구하면 된다.
